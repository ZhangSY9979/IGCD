...
class SplitNet(torch.nn.Module):  # 分离网络
...
class SplitModlue:
    ...
    def show_OnN(self, y, v, nb_classes, pth_result, pth_name, thres=0., is_hist=False, iter=0, loss=None):
        ...
        for j in range(len(y)):
            if y[j] < nb_classes: # 标签值小于初始，认定为旧类
                if loss is not None:
                    o.append(loss[j])
                else:
                    o.append(v[j])    
    ...
    def generate_dataset(self, dataset, index, index_target=None, target=None, index_target_new=None):
        # 返回一个新的数据集，这个数据集实现了把旧类的标签作为0，新类的标签为1
        dataset_ = copy.deepcopy(dataset)
        dataset_.targets[index_target] = 0
        dataset_.targets[index_target_new] = 1
        save_index = []
        for i in range(len(dataset_.targets)):
            if i not in index:
                save_index.append(i)
        dataset_.data = dataset_.data[save_index]
        dataset_.targets = dataset_.targets[save_index]
        return dataset_
    ...
    def calc_GMM_cross(self, total_loss, y, 
        ...
        total_loss_org = (total_loss - total_loss.min()) / (total_loss.max() - total_loss.min())  # 归一化一下
    ...
    def split_old_and_new(self,
        ...
        m_org, y_true = self.evaluate_cos_(main_model, dl_ev)  # 特征和真实标签
        cos_sim = F.linear(l2_norm(m_org), l2_norm(proxy.proxies))  # 计算特征和代理的余弦相似度
        v, y_p = torch.max(cos_sim, dim=1)  # 获取最大值以及对应的label

        y_p_noise = y_p.cpu().detach().numpy()   # 假的
        v_arr = v.cpu().detach().numpy()  # 值
        y_true_arr = y_true.cpu().detach().numpy()  # 真的
...
#                    print(y)
                    ...
                    total_loss = self.soft_cross_entropy(y_bin_pridict, y)  # 训练分离网络
            ...
            m_, y_true, _ = self.predict_batchwise_split(main_model_, self.siplitnet, dl_ev)  # 伪标签和真实标签

            y_true_arr = y_true.cpu().detach().numpy()
            m_ = torch.softmax(m_, dim=1)   # ？？？？感觉不需要softmax了呢
            y_0_1 = []
