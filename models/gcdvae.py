...
class GCDVAE(IncrementalModel):
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, args):
        super(GCDVAE, self).__init__(args)
        ...
        self.bate = torch.zeros(100).to(self.device)
    ...
    def train_first(self, dataset, train_loader):
        ...
        for category in categories:
            self.bate[category] = self.set_score(train_loader, category, 0)
    ...
    def test_first(self, dataset, test_loaders):
        ...
        W_label1 = []
        W_pre1 = []
        # print(test_loaders.dataset.data.shape)
        ...
        # all_outputs = torch.tensor(all_outputs).cpu().numpy()
        # all_labels = torch.cat(all_labels, dim=0).numpy()
        # preds = torch.cat(preds, dim=0).numpy()
        # np.save("dist", all_outputs)
    ...
    def test_cluster(self, dataset, test_loader):
        ...
        if self.current_task == 3:
            np.save("gcd_true_label", all_labels)
            np.save("gcd_pred_label", preds)
    ...
    def train_category(self, data_loader, category: int, epoch_id):  # 这个是要修改的，这个目前的都是第一阶段的训练方式，而且
        ...
        for i, data in enumerate(data_loader):  # 一个任务全部的样本，因此有正样本，有负样本
            ...
            pos_loss = torch.relu(dist[labels == category] - self.r_intra)  # 类内损失
            ...
            if self.current_task > 0:
                ...
                if self.isPseudo:
                    ...
                    pseudo_loss = self.lambda2 * torch.relu(self.r_inter - pseudo_dist)  # 越大越好
